...
import { HABITS_COLORS, MAX_DAYS_VIEW, ANALYSIS_DAYS, h_uid } from '../constants/habits';
import {
  H_DOW,
  h_dayList,
  getHabitStatus,
...
export default function HabitsScreen() {
  ...
  const [statusModal, setStatusModal] = useState<{ taskId: string; date: string } | null>(null);
  ...
  function handleCellPress(dIdx: number, taskId: string, dateKey: string) {
    const locked = dIdx > 1; // only today (0) and yesterday (1) editable
    if (locked) {
      showLockedToast();
      return;
    }
  ...
  function handleStatusSelect(status: HabitStatus) {
    ...
    const next = setHabitStatus(entries, statusModal.date, statusModal.taskId, status);
    persistEntries(next);
    setStatusModal(null);
  }
  ...
  const ringColor =
    summary.pct === 100 ? colors.green : summary.pct > 0 ? colors.yellow : colors.hTextFaint;

  const statusModalTask = statusModal ? tasks.find((t) => t.id === statusModal.taskId) : null;
  const statusModalDateLabel = statusModal
    ...
    : '';
  const statusModalCurrent = statusModal
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerBtns}>
          <TouchableOpacity style={styles.iconBtn} onPress={() => setAddModalVisible(true)}>
            <Ionicons name="add" size={22} color={colors.hText} />
          </TouchableOpacity>
          <TouchableOpacity
      ...
      {editMode ? (
        ...
        <ScrollView contentContainerStyle={styles.editScroll}>
          <Text style={styles.editHint}>Use ▲▼ to reorder · Tap trash to delete</Text>
          {tasks.map((task, idx) => (
            <View key={task.id} style={styles.editRow}>
              ...
              <TouchableOpacity
                ...
                style={[styles.editArrowBtn, idx === 0 && styles.editArrowDisabled]}
              >
              ...
              >
                <Ionicons name="chevron-down" size={16} color={colors.hTextDim} />
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => handleDeleteHabit(task.id)}
                style={styles.editTrashBtn}
              >
                <Ionicons name="trash-outline" size={16} color={colors.hDanger} />
              </TouchableOpacity>
            </View>
          ))}
          <TouchableOpacity style={styles.addRowBtn} onPress={() => setAddModalVisible(true)}>
            <Ionicons name="add" size={18} color={colors.hTextDim} />
            <Text style={styles.addRowBtnText}>Add a habit</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.doneEditingBtn} onPress={() => setEditMode(false)}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {tasks.length === 0 ? (
            <View style={styles.emptyState}>
              <Ionicons name="stats-chart-outline" size={28} color={colors.hTextFaint} />
            ...
            <View style={styles.gridWrap}>
              ...
              <View style={styles.nameCol}>
                ...
                {tasks.map((task) => {
                  const streak = computeHabitStreaks(entries, task.id, ANALYSIS_DAYS);
                  return (
                    <View key={task.id} style={[styles.nameCell, { height: ROW_HEIGHT }]}>
                      <View
                        style={[
                          styles.bulletDot,
                          { backgroundColor: streak.current > 0 ? task.color : 'transparent' },
                        ]}
                      />
                      <View style={{ flex: 1 }}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View>
                  ...
                  {tasks.map((task) => (
                    <View key={task.id} style={[styles.dataRow, { height: ROW_HEIGHT }]}>
                      {days.map((d, dIdx) => {
                        const dateKey = fmtDate(d);
                        const status = getHabitStatus(entries, dateKey, task.id);
                        const locked = dIdx > 1;
                        return (
                          <TouchableOpacity
                            key={dateKey}
                            style={[styles.statCell, { width: DAY_COL_WIDTH }]}
                            onPress={() => handleCellPress(dIdx, task.id, dateKey)}
                          ...
                          >
                            <Ionicons
                              ...
                              style={locked && status === 'unknown' ? { opacity: 0.35 } : undefined}
                            />
                          </TouchableOpacity>
                        );
                      })}
      ...
      {toastVisible && (
        <Animated.View style={[styles.toast, { opacity: toastOpacity }]}>
          <Text style={styles.toastText}>Only today and yesterday can be edited</Text>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  emptyText: { color: colors.hTextFaint, fontSize: 13, textAlign: 'center' },
  ...
  headCell: { alignItems: 'center', justifyContent: 'flex-end', paddingBottom: 4 },
  headCellToday: { backgroundColor: colors.hSurface2, borderRadius: 8 },
  dow: { color: colors.hTextFaint, fontSize: 8.5, fontWeight: '700' },
  dnum: { color: colors.hTextDim, fontSize: 12, fontWeight: '600', marginTop: 1 },

  dataRow: { flexDirection: 'row', borderTopWidth: 1, borderTopColor: colors.hBorder },
